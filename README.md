...
2) Выбираем путь где будет хранится compose.yaml (можно использовать путь созданный выше для данных)
3) Источник - создать docker-compose.yml
4) В поле ввода вводим данные <a id="A1">отсюда</a>. Не забываем заменить путь на свой.
...
Когда увидели данную надпись, значит проект успешно установился. Можно перейти к его <a id="A4">настройке</a>.
...
- Telegram settings
  ...
  4) Нажимаем тест, в чат придёт тестовое сообещние, если его получили - значит телеграм настроен верно.
...
- **Отправка кусками** - Система будет отсылать видеофрагменты 15-секундными интервалами, до тех пор, пока движение есть.
